...
for j = 1:length(spacingList)
    ...
    for i = 1:length(geometryList)
        ...
        [ ~, offset, ~, tempPE, ~, ~, ~, ~, ~, ~ ] = processChain(tau, phi, rho, wavelengthFactor, alpha, delta, gamma, beta, epsilon,  epsilonPush, tau0Push, taufPush, epsilonPull, tau0Pull, taufPull);

        ratioList(i, j) = alpha(end)/(2*pi) - (i-1)/N;
        PE(i, j) = tempPE(end)/N;
...
xlabel([ '(spring length)/\lambda ' char(8722) ' (# solitons)/N' ])
...
makePrint('../Pictures', 'EvA2', 'pdf', true)
